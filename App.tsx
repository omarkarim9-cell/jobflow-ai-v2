...
export const App: React.FC = () => {
  ...
  const handleBulkGenerate = async () => {
    ...
    for (const id of idsToProcess) {
        ...
        if (!job) continue;
        setBulkProgress(`Preparing ${job.company}...`);
        try {
            ...
            if (token) saveJobToDb(updatedJob, token);
            generatedResults.push({ jobId: job.id, jobTitle: job.title, company: job.company, resume: newResume, letter: newLetter, applicationUrl: job.applicationUrl });
        } catch (e) { 
          console.error(e); 
        }
    }
  ...
  if (!isLoaded || loading) return <div className="h-screen flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-indigo-600"/></div>;
  if (!isSignedIn) return <Auth onLogin={() => {}} onSwitchToSignup={() => {}} />;

  const scannedJobs = jobs.filter(j => j.status === JobStatus.DETECTED);
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <aside className="w-64 bg-white border-e border-slate-200 flex flex-col shrink-0 z-20">
        ...
        <div className="p-4 border-t border-slate-200 mt-auto">
             <button onClick={() => signOut()} className="w-full flex items-center px-3 py-2.5 rounded-lg bg-slate-100 text-slate-600 hover:bg-red-50 hover:text-red-600 transition-colors font-medium">
